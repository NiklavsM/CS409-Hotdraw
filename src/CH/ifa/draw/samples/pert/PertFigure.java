...
public class PertFigure extends CompositeFigure {
    ...
    private Vector<PertFigure> fPreTasks;
    private Vector<PertFigure> fPostTasks;
    ...
    public int start() {
        int start = 0;
        Enumeration<PertFigure> i = fPreTasks.elements();
    ...
    private void initialize() {
        fPostTasks = new Vector<PertFigure>();
        fPreTasks = new Vector<PertFigure>();
    ...
    public void notifyPostTasks() {
        Enumeration<PertFigure> i = fPostTasks.elements();
        while (i.hasMoreElements())
    ...
    public void writeTasks(StorableOutput dw, Vector<PertFigure> v) {
        dw.writeInt(v.size());
        Enumeration<PertFigure> i = v.elements();
        while (i.hasMoreElements())
    ...
    public Vector<PertFigure> readTasks(StorableInput dr) throws IOException {
        int size = dr.readInt();
        Vector<PertFigure> v = new Vector<PertFigure>(size);
        for (int i=0; i<size; i++)
            v.addElement((PertFigure)dr.readStorable());
        return v;
    }
}
